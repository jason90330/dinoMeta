...
def get_args_parser():    
    ...
    parser.add_argument('--total_iter', default=1536, type=int,
        help='Per-GPU batch-size : number of distinct images loaded on one GPU.')        
    parser.add_argument('--sub_step_times', default=4, type=int,
        help='Per-GPU batch-size : number of distinct images loaded on one GPU.')
    ...
    parser.add_argument('--output_dir', default="output/ssl_reptile_speed/", type=str, help='Path to save logs and checkpoints.')
...
def train_dino(args):
    ...
    dataset_dark = customData(illumination_domain=4,
                                ...
                                data_transforms=transform,
                                phase="ssl")
    bs_live, bs_normal, bs_strong, bs_back, bs_dark = \
                    len(dataset_live)/args.total_iter, len(dataset_normal)/args.total_iter,\
                    len(dataset_strong)/args.total_iter, len(dataset_back)/args.total_iter, \
                    len(dataset_dark)/args.total_iter
    bs_live, bs_normal, bs_strong, bs_back, bs_dark = round(bs_live), round(bs_normal), round(bs_strong), round(bs_back), round(bs_dark)                    
    ...
    data_loader_live = torch.utils.data.DataLoader(
        ...
        batch_size=bs_live,#args.batch_size_per_gpu,
...
def train_one_epoch(student, teacher, teacher_without_ddp, embed_dim, dino_loss, 
    ...
    for step in range(iternum):
        ...
        for index in meta_train_list: # iterate all tasks, each task include meta-train and meta-test
            ...
            Meta-train
            """
            batch_images_train = catimglist[index]
            # batch_images_train = [im.cuda(non_blocking=True) for im in batch_images_train]
            step_batch_train = int(len(batch_images_train[0])/args.sub_step_times)
            for sub_step in range(args.sub_step_times):
                images = []
                for idx in range(0, args.local_crops_number+2):
                    images.append(batch_images_train[idx][sub_step*step_batch_train : (sub_step+1)*step_batch_train])
                images = [im.cuda(non_blocking=True) for im in images]
                # images =[batch_images[0][step*step_batch : (step+1)*step_batch],
                #          batch_images[1][step*step_batch : (step+1)*step_batch],
                #          batch_images[2][step*step_batch : (step+1)*step_batch],
                #          batch_images[3][step*step_batch : (step+1)*step_batch],
                #          batch_images[4][step*step_batch : (step+1)*step_batch]]
                # images = batch_images[:, step*step_batch : (step+1)*step_batch]
                ...
                batch_images_test = catimglist[meta_test_index]
                step_batch_test = int(len(batch_images_test[0])/args.sub_step_times)
                for idx in range(0, args.local_crops_number+2):
                    images.append(batch_images_test[idx][sub_step*step_batch_test : (sub_step+1)*step_batch_test])
                images = [im.cuda(non_blocking=True) for im in images]
