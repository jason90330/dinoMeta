        ...
        for eval_spoof_type in range(1, 14):            
            ...
            for idx,(out, tar) in enumerate(zip(probablity, target)):
                ...
                if tar == 0:
                    ...
                    category_score.append(out[0]) #存入 live index 位置的 probability
                    if pred == tar: # 如果預測正確的話
                        category_pred.append(0)#就視為 live
                    else:
                        category_pred.append(1)#否則就是預測為 spoof

                elif tar == eval_spoof_type:
                    category_target.append(1)
                    category_score.append(out[tar]) #取出那個 spoof target index 位置的 probability
                    if pred == tar: # 如果預測正確的話
                        category_pred.append(1)#就視為 spoof
                    else:
                        category_pred.append(0)#否則就視為預測為 live(但有瑕疵，也有可能是預測為其他type的spoof)
